...
const TOKEN_URL = 'https://pastebin.com/raw/6KCPeQEk';
const REPO_OWNER = 'Bintz-debug';
const REPO_NAME = 'bintzeg';
const FILE_PATH = 'bintz.json';
const BRANCH = 'main';

const apiBaseUrl = `https://api.github.com/repos/${REPO_OWNER}/${REPO_NAME}/contents/${FILE_PATH}`;
...
const getGithubToken = async () => {
    try {
        ...
        console.error("Gagal mengambil token GitHub:", err.message);
        return null;
    }
};
...
const getFileData = async () => {
    ...
    if (!GITHUB_TOKEN) throw new Error("Token GitHub tidak tersedia");
...
const valid = async (nomor) => {
    try {
        ...
        const found = content.find(u => u.nomor === nomor);
        if (found) {
            console.log(`Acces Diterima, Nomor ${nomor} Diizinkan`);
            return 'Valid';
        } else {
            console.log(`Acces Ditolak, Nomor ${nomor} Tidak Diizinkan Oleh BintzGE`);
